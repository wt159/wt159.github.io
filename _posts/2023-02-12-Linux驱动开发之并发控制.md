...
3. `init_completion(&comp)`：初始化完成量，将其计数器设置为0，并将等待队列清空。

4. `wait_for_completion(&comp)`：等待完成量的计数器达到0。如果计数器不为0，则当前进程将被阻塞并加入等待队列中，直到计数器为0。

5. `wait_for_completion_interruptible(&comp)`：与`wait_for_completion(&comp)`类似，但是如果进程接收到中断信号，则会返回-EINTR错误。

6. `complete(&comp)`：增加完成量的计数器，并唤醒等待队列中的进程。

7. `complete_all(&comp)`：增加完成量的计数器，并唤醒等待队列中的所有进程。
