...
### adb调用audioflinger的setParameters函数

```shell
# audio setparameter
service call media.audio_flinger 21 i32 0 s16 "key=value" # setparameter
service call media.audio_flinger 22 i32 0 s16 "key" # getparameter
```

* `frameworks/av/media/libaudioclient/aidl/android/media/IAudioFlingerService.aidl` `21` `22`是binder服务里函数的排序
...
手机和车机公共音频调试命令
dumpsys media.audio_flingerdumpsys 
media.audio_policy
dumpsys audio
车机独享的命令部分
dumpsys car_service
...
adb shell setprop aaudio.mmap_exclusive_policy 1
```
